...
MAX_POLYGON_DEVIATION_PX = 1.0


def mask_point_spacing_ratio(app):
    var = getattr(app, 'mask_point_spacing_var', None)
    if var is None:
        return 0.0
    try:
...
def _max_offset_from_segment(points, start, end):
    if points.shape[0] == 0:
        return 0.0
    seg = end - start
    ...
    rel = points - start
    if length == 0.0:
        return float(np.hypot(rel[:, 0], rel[:, 1]).max())
    cross = seg[0] * rel[:, 1] - seg[1] * rel[:, 0]
    return float(np.abs(cross).max() / length)


def decimate_contour_by_spacing(contour, spacing_ratio,
                                max_deviation=MAX_POLYGON_DEVIATION_PX):
    pts = np.asarray(contour).reshape(-1, 2).astype(np.float64)
    n = pts.shape[0]
    if spacing_ratio <= 0 or n <= 4:
        return np.asarray(contour).reshape(-1, 1, 2).astype(np.int32)

    spacing = spacing_ratio * float(cv2.arcLength(
        pts.reshape(-1, 1, 2).astype(np.int32), True))
    if spacing <= 0:
        return np.asarray(contour).reshape(-1, 1, 2).astype(np.int32)
    ...
    anchor = 0
    for i in range(1, n):
        step = pts[i] - pts[anchor]
        if float(np.hypot(step[0], step[1])) >= spacing:
            kept.append(i)
            anchor = i
            continue
        if _max_offset_from_segment(pts[anchor + 1:i], pts[anchor], pts[i]) > max_deviation:
            kept.append(i)
            anchor = i

    if len(kept) > 3 and kept[-1] != n - 1:
        tail = pts[kept[-1] + 1:]
        if _max_offset_from_segment(tail, pts[kept[-1]], pts[0]) > max_deviation:
            kept.append(n - 1)

    if len(kept) < 3:
        return np.asarray(contour).reshape(-1, 1, 2).astype(np.int32)
    return pts[kept].reshape(-1, 1, 2).astype(np.int32)


def decimate_contours_by_spacing(contours, spacing_ratio,
                                 max_deviation=MAX_POLYGON_DEVIATION_PX):
    return [decimate_contour_by_spacing(c, spacing_ratio, max_deviation) for c in contours]


def extract_polygon_contours(mask_uint8):
    contours, hierarchy = cv2.findContours(mask_uint8, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
    if not contours:
        return [], []
    if hierarchy is None:
        return list(contours), [[] for _ in contours]

    outers, holes, slot_of = [], [], {}
    for idx, contour in enumerate(contours):
        if hierarchy[0][idx][3] == -1:
            slot_of[idx] = len(outers)
            outers.append(contour)
            holes.append([])
    for idx, contour in enumerate(contours):
        parent = hierarchy[0][idx][3]
        if parent in slot_of:
            holes[slot_of[parent]].append(contour)
    return outers, holes


def attach_holes_to_contour(outer, holes, min_area=10):
    from scipy.spatial.distance import cdist

    ring = np.asarray(outer).reshape(-1, 2).astype(np.int64)
    if not holes:
        return ring.reshape(-1, 1, 2).astype(np.int32)

    outer_is_ccw = cv2.contourArea(ring.reshape(-1, 1, 2).astype(np.int32), True) > 0

    usable = []
    for hole in holes:
        arr = np.asarray(hole).reshape(-1, 2).astype(np.int64)
        if arr.shape[0] < 3:
            continue
        signed = cv2.contourArea(arr.reshape(-1, 1, 2).astype(np.int32), True)
        if abs(signed) < min_area:
            continue
        if (signed > 0) == outer_is_ccw:
            arr = arr[::-1]
        usable.append((abs(signed), arr))

    for _, hole in sorted(usable, key=lambda item: item[0], reverse=True):
        dm = cdist(ring, hole, 'euclidean')
        flat = int(np.argmin(dm))
        im, ic = divmod(flat, dm.shape[1])
        loop = np.concatenate([hole[ic:], hole[:ic], hole[ic:ic + 1]], axis=0)
        ring = np.concatenate([ring[:im + 1], loop, ring[im:]], axis=0)

    return ring.reshape(-1, 1, 2).astype(np.int32)


def build_mask_polygon(mask, spacing_ratio=0.0, min_contour_area=0.0, min_area=10,
                       max_deviation=MAX_POLYGON_DEVIATION_PX):
    arr = np.asarray(mask)
    mask_uint8 = arr.astype(np.uint8) if arr.dtype == bool else (arr > 0).astype(np.uint8)
    outers, holes = extract_polygon_contours(mask_uint8)

    rings = []
    for outer, own_holes in zip(outers, holes):
        if cv2.contourArea(outer) < min_contour_area:
            continue
        thinned_outer = decimate_contour_by_spacing(outer, spacing_ratio, max_deviation)
        thinned_holes = [
            decimate_contour_by_spacing(h, spacing_ratio, max_deviation)
            for h in own_holes if cv2.contourArea(h) >= max(min_contour_area, min_area)
        ]
        rings.append(attach_holes_to_contour(thinned_outer, thinned_holes, min_area=min_area))

    if not rings:
        return None
    return merge_contours_into_single_polygon(rings, min_area=min_area)
