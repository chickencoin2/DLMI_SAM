...
from .customutil import (
    ...
    build_mask_polygon, mask_point_spacing_ratio,
)
...
def _resolve_save_paths(app, frame_idx, label_subdir, label_ext, image_ext="jpg",
                        include_image=True, save_dir_override=None, force_stem=None):
    ...
    if force_stem:
        return (save_dir, final_base_filename,
                os.path.join(labels_dir, f"{force_stem}.{label_ext}"),
                os.path.join(images_dir, f"{force_stem}.{image_ext}") if images_dir else None)
    ...
    for sam_id, obj_data_original in masks_data_for_frame.items():
        ...
        if labeling_mode == "Bounding Box":
            ...
            if filter_small_contours and filter_small_contours.get():
                ...
                base_area = h * w if base_type == "image" else np.sum(mask)
                min_contour_area = base_area * threshold_ratio

            merged_polygon_contour = build_mask_polygon(
                mask,
                spacing_ratio=mask_point_spacing_ratio(app),
...
def save_labelme_json(app, frame_pil_image_rgb, frame_idx, masks_data_for_frame, base_filename_prefix,
                      is_both_mode=False, source_image_path=None, include_pose=False,
                      write_empty=False):
    ...
    h, w = frame_pil_image_rgb.height, frame_pil_image_rgb.width
    pose_source_masks = masks_data_for_frame
    ...
    if include_pose:
        for sam_id, obj_data in pose_source_masks.items():
            ...
            if not pose_pts:
                continue
            pose_label = obj_data.get('custom_label', default_label_value)
            if not pose_label or pose_label.strip() == "" or pose_label.lower() == "object":
                pose_label = f"Object_{sam_id}"
            for kpt_idx, point in enumerate(pose_pts):
                labelme_data["shapes"].append({
                    "label": pose_label,
                    "points": [[float(point['x']), float(point['y'])]],
                    "group_id": sam_id if isinstance(sam_id, int) else None,
                    "description": str(kpt_idx),
                    "shape_type": "point",
                    "flags": {"visibility": int(point.get('visibility', 2))},
                })

    if labelme_data["shapes"] or write_empty:
        try:
...
def save_yolo_pose_format(app, frame_pil_image_rgb, frame_idx, masks_data_for_frame, base_filename_prefix,
    ...
    forced_stem = (os.path.splitext(os.path.basename(source_image_path))[0]
                   if source_image_path else None)
    if use_pose_root:
        ...
        _save_dir, _base, label_filepath, pose_image_target = _resolve_save_paths(
            ...
            include_image=True, save_dir_override=pose_root, force_stem=forced_stem,
        )
    else:
        ...
        _save_dir, _base, label_filepath, pose_image_target = _resolve_save_paths(
            app, frame_idx, label_subdir=labels_subdir, label_ext="txt",
            include_image=True, force_stem=forced_stem,
        )
...
def save_frame_dispatch(app, frame_pil, frame_idx, masks, video_name, pose_subdir=None,
    ...
    if fmt in ("labelme", "both") or always_json:
        # In 'both' mode reuse the JPEG save_yolo_format already encoded instead of re-encoding.
        json_beside_yolo = fmt in ("both", "yolo")
        save_labelme_json(
            app, frame_pil, frame_idx, masks, video_name,
            is_both_mode=json_beside_yolo,
            source_image_path=yolo_image_path if json_beside_yolo else None,
            include_pose=always_json,
            write_empty=always_json,
        )
