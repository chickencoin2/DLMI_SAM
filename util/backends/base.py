...
MASK_PROMPT_LOGIT_SCALE = 20.0
MASK_PROMPT_LOGIT_BIAS = -10.0


def mask_to_prompt_logits(mask, size_hw, device, dtype=None):
    import torch
    ...
    if m.ndim != 2:
        m = np.squeeze(m)
    if m.ndim != 2:
        raise ValueError(f"mask prompt must be 2-D, got shape {m.shape}")
    t = torch.from_numpy((m > 0).astype(np.float32))[None, None].to(device)
    if tuple(t.shape[-2:]) != tuple(size_hw):
        t = F.interpolate(t, size=tuple(size_hw), mode="bilinear",
                          align_corners=False, antialias=True)
    t = t * MASK_PROMPT_LOGIT_SCALE + MASK_PROMPT_LOGIT_BIAS
...
class SamBackend(abc.ABC):
    ...
    def image_predict_mask(self, frame, mask: np.ndarray, *,
                           multimask_output: bool = True) -> np.ndarray:
        self._todo("image_predict_mask")
