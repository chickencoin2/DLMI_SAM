...
class HFBackend(SamBackend):
    ...
    def image_predict_mask(self, frame, mask, *, multimask_output=True) -> np.ndarray:
        app = self.app
        model = app.tracker_model
        processor = app.tracker_processor
        if model is None or processor is None:
            raise BackendLoadError("HuggingFace tracker model is not loaded.")
        pil = to_pil_rgb(frame)
        w, h = pil.size
        ...
        input_masks = mask_to_prompt_logits(
            mask, model.prompt_encoder.mask_input_size, self.device, pixel_values.dtype)
        image_forward = getattr(model, "_single_frame_forward", None) or model
        with torch.inference_mode():
            outputs = image_forward(pixel_values=pixel_values, input_masks=input_masks,
        ...
        upscaled = processor.post_process_masks([pred], original_sizes=[[h, w]], binarize=False)[0]
        return upscaled[0, 0].float().cpu().numpy()
