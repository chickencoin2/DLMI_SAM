...
from .base import SamBackend, BackendLoadError, DetectResult, to_pil_rgb, mask_to_prompt_logits
...
class GitBackend(SamBackend):
    ...
    # Mask prompt input after image predict
    # It may be helpful when human labeling is inaccurate or when images and objects are simple.
    def image_predict_mask(self, frame, mask, *, multimask_output=True) -> np.ndarray:
        if not self._loaded:
            raise BackendLoadError("GitBackend.image_predict_mask before load()")
        inst = getattr(self._img_model, "inst_interactive_predictor", None)
        if inst is None:
            raise BackendLoadError("official image model was built without instance interactivity")
        pil = to_pil_rgb(frame)
        mask_size = tuple(inst.model.sam_prompt_encoder.mask_input_size)
        low_res = mask_to_prompt_logits(mask, mask_size, self._device_str, torch.float32)
        with self._autocast_ctx():
            state = self._img_proc.set_image(pil)
            masks, ious, _ = self._img_model.predict_inst(
                state, mask_input=low_res[0],
                multimask_output=multimask_output, return_logits=True)
        masks = np.asarray(masks, dtype=np.float32)
        if masks.ndim == 2:
            return masks
        ious = np.asarray(ious, dtype=np.float32).reshape(-1)
        best = int(np.argmax(ious)) if ious.size == masks.shape[0] else 0
        return masks[best]
