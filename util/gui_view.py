...
class AppView:
    ...
    def _setup_widgets(self):
        ...
        self.btn_save_current_frame = tk.Button(
            self.top_frame, text="Save Current Frame",
            command=self.app.save_current_frame_labels, state=tk.DISABLED, bg="#c8e6c9")
        ...
        # sam3 mask prompt input with polygon/paint
        self.btn_mask_input = tk.Button(
            self.paint_size_frame, text="Mask prompt Input",
            command=self.app.input_mask_to_sam if hasattr(self.app, 'input_mask_to_sam') else None,
            state=tk.DISABLED, bg="#bbdefb"
        )
        self.btn_low_data_inject_paint = tk.Button(
            self.paint_size_frame, text="Inject Data",
            command=self.app.inject_low_level_mask_prompt if hasattr(self.app, 'inject_low_level_mask_prompt') else None,
            state=tk.DISABLED, bg="#e1bee7"
        )
        ...
        self.mask_spacing_frame = tk.Frame(self.batch_control_frame)
        self.mask_spacing_label = tk.Label(
            self.mask_spacing_frame, text="Min Point Gap(%):", font=("TkDefaultFont", 8))
        self.mask_spacing_entry = tk.Entry(
            self.mask_spacing_frame, width=6, justify=tk.CENTER
        )
        self.mask_spacing_entry.insert(
            0, f"{self.app.mask_point_spacing_var.get() * 100:g}")
        self.mask_spacing_entry.bind("<Return>", self._on_mask_spacing_change)
        self.mask_spacing_entry.bind("<FocusOut>", self._on_mask_spacing_change)
    ...
    def _layout_widgets(self):
        ...
        self.btn_save_current_frame.pack(side=tk.LEFT, padx=2)
        ...
        self.btn_mask_input.pack(side=tk.LEFT, padx=(10, 2))
        self.btn_low_data_inject_paint.pack(side=tk.LEFT, padx=2)
        ...
        self.mask_spacing_frame.pack(pady=2, padx=5, fill=tk.X)
        self.mask_spacing_label.pack(side=tk.LEFT)
        self.mask_spacing_entry.pack(side=tk.LEFT, padx=(5, 2))

        self.pose_control_frame.grid(row=6, column=0, padx=5, pady=5, sticky='ew')
    ...
    def set_propagate_button_states(self, is_propagating):
        try:
            if is_propagating:
                ...
                self.update_current_frame_save_button_state()
    ...
    def update_current_frame_save_button_state(self):
        enabled = (
            getattr(self.app, 'current_cv_frame', None) is not None
            and not getattr(self.app, 'current_frame_save_in_progress', False)
        )
        try:
            self.btn_save_current_frame.config(state=tk.NORMAL if enabled else tk.DISABLED)
        except tk.TclError:
            pass
    ...
    def update_obj_id_info_label(self):
        ...
        self.update_mask_input_button_state()
    ...
    def set_ui_element_state(self, element_name, state):
        widget_map = {
            ...
            "btn_save_current_frame": self.btn_save_current_frame,
        }
    ...
    def _on_mask_spacing_change(self, event=None):
        try:
            percent_value = float(self.mask_spacing_entry.get())
            ratio_value = max(0.0, percent_value / 100.0)
            self.app.mask_point_spacing_var.set(ratio_value)
            logger.info(f"Polygon point spacing changed: {percent_value}% (ratio: {ratio_value})")
        except ValueError:
            logger.warning("Polygon point spacing must be a number.")
            current_ratio = self.app.mask_point_spacing_var.get()
            self.mask_spacing_entry.delete(0, tk.END)
            self.mask_spacing_entry.insert(0, f"{current_ratio * 100:g}")
    ...
    def _on_low_level_api_toggle(self):
        ...
        self.update_low_data_inject_button_state()
    ...
    def update_low_data_inject_button_state(self):
        ...
        state = 'normal' if (is_api_enabled and has_masks) else 'disabled'
        self.btn_low_data_inject.config(state=state)
        if hasattr(self, 'btn_low_data_inject_paint'):
            self.btn_low_data_inject_paint.config(state=state)

    def update_mask_input_button_state(self):
        if not hasattr(self, 'btn_mask_input'):
            return
        app = self.app
        has_pending_paint = bool(
            getattr(app, 'paint_mode_active', False)
            and getattr(app, 'paint_stroke_mask', None) is not None
            and app.paint_stroke_mask.any()
        )
        has_polygon_obj = any(
            data.get('is_polygon_object', False)
            for data in getattr(app, 'tracked_objects', {}).values()
        )
        self.btn_mask_input.config(state='normal' if (has_pending_paint or has_polygon_obj) else 'disabled')
